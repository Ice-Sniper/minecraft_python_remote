...
# mc = Minecraft.create(address='nao2g008', port=param.PORT_MC)
mc = Minecraft.create(port=param.PORT_MC)
mc.postToChat("axis module loaded")

def draw_XYZ_axis(mc=mc, clear=False, wait=0.1):

    if (clear==True):
        blockTypeIdX = param.AIR
        blockTypeIdY = param.AIR
        blockTypeIdZ = param.AIR
    else:
    ...
    while y <= param.AXIS_TOP - 5:
        ...
        if y < param.AXIS_Y_V_ORG:
            ...
            y += 1
            sleep(wait)
    blockTypeId = param.SEA_LANTERN_BLOCK
